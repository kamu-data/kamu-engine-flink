...
/**
 * Copied from ParquetInputFormat for the sole purpose of substituting implementation of the RowReadSupport
 */
public class ParquetRowInputFormatEx extends FileInputFormat<Row>
        implements CheckpointableInputFormat<FileInputSplit, Tuple2<Long, Long>>,
        ResultTypeQueryable<Row> {

    private static final long serialVersionUID = 1L;

    private static final Logger LOG = LoggerFactory.getLogger(ParquetRowInputFormatEx.class);
     ...
     */
    public ParquetRowInputFormatEx(Path path, MessageType messageType) {
        super(path);
        this.expectedFileSchema = Preconditions.checkNotNull(messageType, "messageType");
        RowTypeInfo rowTypeInfo =
                (RowTypeInfo) ParquetSchemaConverterEx.fromParquetType(expectedFileSchema);
    ...
    @Override
    public Row nextRecord(Row e) throws IOException {
        if (reachedEnd()) {
            return null;
        }
    ...
    private MessageType getReadSchema(MessageType fileSchema, Path filePath) {
        RowTypeInfo fileTypeInfo = (RowTypeInfo) ParquetSchemaConverterEx.fromParquetType(fileSchema);
