 ...
 */
public class CustomFileReaderOperator<OUT, T extends TimestampedInputSplit> extends AbstractStreamOperator<OUT>
		implements OneInputStreamOperator<T, OUT>, OutputTypeConfigurable<OUT> {

	private static final long serialVersionUID = 1L;

	private static final Logger LOG = LoggerFactory.getLogger(CustomFileReaderOperator.class);

	private enum ReaderState {
		IDLE {
			@Override
			public boolean prepareToProcessRecord(CustomFileReaderOperator<?, ?> op) {
				throw new IllegalStateException("not processing any records in IDLE state");
			}

			@Override
			public void onNoMoreData(CustomFileReaderOperator<?, ?> op) { op.doneProcessing(); }
		},
		/**
		...
		OPENING { // the split was added and message to itself was enqueued to process it
			@Override
			public boolean prepareToProcessRecord(CustomFileReaderOperator<?, ?> op) throws IOException {
				if (op.splits.isEmpty()) {
					op.switchState(ReaderState.IDLE);
					return false;
				} else {
					((CustomFileReaderOperator) op).loadSplit(op.splits.poll());
		...
		READING {
			...
			@Override
			public void onNoMoreData(CustomFileReaderOperator<?, ?> op) {
				op.doneProcessing();
				op.switchState(ReaderState.IDLE);
			}
		},
		 ...
		 */
		public abstract boolean prepareToProcessRecord(CustomFileReaderOperator<?, ?> op) throws IOException;
	...
	public void open() throws Exception {
		...
		if (!prevWatermark.equals(Instant.MIN)) {
			...
			sourceContext.emitWatermark(new Watermark(prevWatermark.toEpochMilli()));
		}
	...
	public void processElement(StreamRecord<T> element) throws Exception {
		...
		if(element.getValue().getSplitNumber() >= 0) {
	...
	private void doneProcessing() {
		LOG.info("{}: No more data", getOperatorName());
		...
		for(Instant instant: explicitWatermarks) {
			...
			sourceContext.emitWatermark(new Watermark(instant.toEpochMilli()));
		}
