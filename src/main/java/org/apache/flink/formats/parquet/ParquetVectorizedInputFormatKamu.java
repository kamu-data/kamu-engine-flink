...
import org.apache.flink.formats.parquet.vector.reader.ParquetSplitReaderUtilKamu;
...
// KAMU: Overriding entire class only to use ParquetSplitReaderUtilKamu
public abstract class ParquetVectorizedInputFormatKamu<T, SplitT extends FileSourceSplit>
        implements BulkFormat<T, SplitT> {

    private static final Logger LOG = LoggerFactory.getLogger(ParquetVectorizedInputFormatKamu.class);
    ...
    public ParquetVectorizedInputFormatKamu(
    ...
    private WritableColumnVector[] createWritableVectors(MessageType requestedSchema) {
        ...
        for (int i = 0; i < projectedTypes.length; i++) {
            columns[i] =
                    ParquetSplitReaderUtilKamu.createWritableColumnVector(
                            batchSize,
                            projectedTypes[i],
                            types.get(i),
                            requestedSchema.getColumns(),
        ...
        private void readNextRowGroup() throws IOException {
            ...
            for (int i = 0; i < types.size(); ++i) {
                if (!unknownFieldsIndices.contains(i)) {
                    columnReaders[i] =
                            ParquetSplitReaderUtilKamu.createColumnReader(
                                    isUtcTimestamp,
                                    projectedTypes[i],
                                    types.get(i),
                                    requestedSchema.getColumns(),
